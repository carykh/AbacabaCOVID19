import java.util.*;
import com.hamoid.*;
String VIDEO_FILENAME = "coronavirus_2021-06-25-line_vid.mp4";
boolean SAVE_VIDEO = true;
int LAST_DAY = 542;
int SPECIAL = 1;
int DELAY_BEFORE_ZOOM = 10;
int ZOOM_LENGTH = 26;
float FRAMES_PER_DAY = 20;   // controls the playback speed of the visualization
...
void drawTitles(){
  ...
  if(expoContinue >= 0){
    ...
    textFont(font,48);
    text("Daily death counts of epidemics by",39,70);
  ...
  textFont(font,30);
  text("Video by Cary Huang and Khosraw Azizi",0,0);
  text("a.k.a. @realCarykh and @AziziKhosraw",0,35);
  textFont(font,24);
  text("Cary's YouTube channel: youtube.com/1abacaba1",0,67);
  popMatrix();
  popMatrix();
}
  ...
  for(int d = 0; d < DIS_COUNT; d++){
    ...
    if(!(d == SPECIAL && currentDayInt >= LAST_DAY+5 && expoContinue < 0) && !derive && d < VIS_SPOT){
      ...
      if(small){
        ...
        fill(cols[d]);
        textAlign(LEFT);
        String deathsDaily = commafy(cases,true);
        int digits = deathsDaily.replace(",","").length();
        String spa = "";
        if(d == SPECIAL){
          spa = " ";
        }
        text(di.vname+": "+spa+deathsDaily,70,10);
        ...
        textFont(font,38);
        text(commafy(cases,true)+" dead daily",4,23);
      }
      popMatrix();
    }
  }
