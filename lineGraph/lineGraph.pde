    ...
    for (int d = 0; d < DIS_COUNT; d++) {
        ...
        if (!(d == SPECIAL && currentDayInt >= LAST_DAY + 5 && expoContinue < 0) && !derive && d < VIS_SPOT) {
            ...
            if (d == SPECIAL && currentDayInt == LAST_DAY + 4) {
                sc = min(max((1 - currentDaySim) / 0.35 f, 0), 1);
            }
            scale(sc);
...
String keyify(long n) {
    if (n < 1000) {
        ...
        if (n % 1000 == 0) {
            ...
            return nf(n / 1000 f, 0, 1) + "K";
        }
    } else if (n < 1000000000) {
        if (n % 1000000 == 0) {
            return (n / 1000000) + "M";
        } else {
            return nf(n / 1000000 f, 0, 1) + "M";
        }
    } else {
        if (n % 1000000000 == 0) {
            return (n / 1000000000) + "B";
        } else {
            return nf(n / 1000000000 f, 0, 1) + "B";
        }
    }
}
...
int dateToDays(String s) {
    ...
    int days = (int)(d1.getTime() / 86400000 L);
    return days;
}
