...
countSounds = [pygame.mixer.Sound('sounds/hurtLouder.wav'), pygame.mixer.Sound('sounds/bell_church_single.wav'),
               pygame.mixer.Sound('sounds/glitter_whoosh.wav')]
...
def addDatumToDatabase(timestamp, countryName, counts):
    recordingString = str(timestamp) + ",datum," + countryName + "," + str(counts[0]) + "," + str(
        counts[1]) + "," + str(counts[2]) + "\n"
...
def drawOther():
    ...
    time_surf = fontSmall.render(
        "Right now: " + datetime.utcfromtimestamp(nowR).strftime('%B %d, %Y %H:%M:%S') + " UTC", 1, (255, 255, 255))
